...
public class SecurityUtil {
	...
	public static String descriptografarAESComAKey(String payloadObjJson) {
		Payload payloadObj = new Gson().fromJson(payloadObjJson, Payload.class);
		String payload = payloadObj.getPayload();
		try {
			byte[] cipherData = Base64.getDecoder().decode(payload);
			byte[] saltData = Arrays.copyOfRange(cipherData, 8, 16);

			MessageDigest md5 = MessageDigest.getInstance("MD5");
			final byte[][] keyAndIV = SecurityUtil.GenerateKeyAndIVToDescript(32, 16, 1, saltData,
	...
	public static String criptografar(String payload) {
		try {
			String password = SecurityUtil.SECRET_KEY_OUT;
			SecureRandom sr = new SecureRandom();
			byte[] salt = new byte[8];
			sr.nextBytes(salt);
			final byte[][] keyAndIV = SecurityUtil.generateKeyAndIVToEncrypt(32, 16, 1, salt,
